...
internal struct dc_unkeyed:Swift.UnkeyedDecodingContainer {
	...
	/// is the unkeyed container at the end of the contents?
	...
	internal init(root:UnsafeMutablePointer<yyjson_val>, logger:Logging.Logger?) throws {
		...
		buildLogger?[metadataKey:"type"] = "dc_unkeyed"
		self.logger = buildLogger
		guard yyjson_get_type(root) == YYJSON_TYPE_ARR else {
			buildLogger?.error("attempted to initialize an unkeyed decoding container on a non-array data type")
		...
		switch self.state {
			...
			case .content(let root):
				...
				switch self.currentIndex < self.length {
					case true:
						#if QUICKJSON_SHOULDLOG
						self.logger?.trace("iterating to next element")
						#endif
						self.state = .content(unsafe_yyjson_get_next(root))
					case false:
						#if QUICKJSON_SHOULDLOG
						self.logger?.trace("no more elements to iterate")
						#endif
						self.state = .end
				}
		}
	...
	internal mutating func decodeNil() throws -> Bool {
		...
		switch self.state {
			case .end:
				#if QUICKJSON_SHOULDLOG
				self.logger?.error("decoder overflow. there are no more items to decode")
				#endif
	...
	internal mutating func decode(_ type:Bool.Type) throws -> Bool {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeBool(logger:logger)
				#else
	...
	internal mutating func decode(_ type:String.Type) throws -> String {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeString(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Double.Type) throws -> Double {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeDouble(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Float.Type) throws -> Float {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeFloat(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Int.Type) throws -> Int {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeInt(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Int8.Type) throws -> Int8 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeInt8(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Int16.Type) throws -> Int16 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeInt16(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Int32.Type) throws -> Int32 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeInt32(logger:logger)
				#else
	...
	internal mutating func decode(_ type:Int64.Type) throws -> Int64 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeInt64(logger:logger)
				#else
	...
	internal mutating func decode(_ type:UInt.Type) throws -> UInt {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeUInt(logger:logger)
				#else
	...
	internal mutating func decode(_ type:UInt8.Type) throws -> UInt8 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeUInt8(logger:logger)
				#else
	...
	internal mutating func decode(_ type:UInt16.Type) throws -> UInt16 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeUInt16(logger:logger)
				#else
	...
	internal mutating func decode(_ type:UInt32.Type) throws -> UInt32 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeUInt32(logger:logger)
				#else
	...
	internal mutating func decode(_ type:UInt64.Type) throws -> UInt64 {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try root.decodeUInt64(logger:logger)
				#else
	...
	internal mutating func decode<T>(_ type:T.Type) throws -> T where T:Decodable {
		#if QUICKJSON_SHOULDLOG
		self.logger?.debug("enter: decode<T>(_:T.Type)")
		defer {
			self.logger?.trace("exit: decode<T>(_:T.Type)")
		}
		#endif
		switch self.state {
			case .end:
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try T(from:decoder(root:root, logger:logger))
				#else
	...
	internal mutating func nestedContainer<NestedKey>(keyedBy type:NestedKey.Type) throws -> KeyedDecodingContainer<NestedKey> where NestedKey : CodingKey {
		#if QUICKJSON_SHOULDLOG
		self.logger?.debug("enter: nestedContainer<NestedKey>(keyedBy:NestedKey.Type)")
		defer {
			self.logger?.trace("exit: nestedContainer<NestedKey>(keyedBy:NestedKey.Type)")
		}
		#endif
		switch self.state {
			case .end:
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try KeyedDecodingContainer(dc_keyed<NestedKey>(root:root, logger:logger))
				#else
	...
	internal mutating func nestedUnkeyedContainer() throws -> UnkeyedDecodingContainer {
		...
		switch self.state {
			...
			case .content(let root):
				#if QUICKJSON_SHOULDLOG
				let decodedValue = try dc_unkeyed(root:root, logger:logger)
				#else
